...
export function ReferrerReport() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const report = await referrerApi.get<ReportData>(`/portal/referrer/orders/${id}/report`);
      setData(report);
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-slate-50">
        ...
        <main className="mx-auto max-w-4xl px-4 py-6">
          ...
          <Link to="/portal/referrer" className="mt-4 thulir-btn thulir-btn-secondary inline-flex">Back to orders</Link>
        </main>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="sticky top-0 z-40 border-b border-brand-900 bg-brand-800 shadow-sm">
        <div className="mx-auto flex h-12 max-w-4xl items-center justify-between px-4">
          <Link to="/portal/referrer" className="text-sm font-bold text-white">Referrer Portal</Link>
      ...
      <main className="mx-auto max-w-4xl px-4 py-6">
        <div className="no-print mb-4 flex flex-wrap items-center justify-between gap-3">
          <Link to="/portal/referrer" className="thulir-btn thulir-btn-secondary">
            Back to orders
          </Link>
